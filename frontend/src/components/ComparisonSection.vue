    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6">
      <div class="text-center mb-12">
        <h2 id="comparison-heading" class="text-2xl sm:text-3xl font-bold text-text-primary mb-3">
          Video Transcriber 与其他视频下载工具对比
        </h2>
        <p class="text-text-secondary text-base max-w-xl mx-auto">
          一张表看清 Video Transcriber 的核心优势，更新于 2026 年 3 月
        </p>
      </div>
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="bg-gray-50 text-text-primary">
              ...
              <th class="px-5 py-3.5 font-semibold text-primary">Video Transcriber</th>
          ...
          <tbody class="bg-white">
            <tr v-for="(row, i) in rows" :key="row.feature" :class="i % 2 === 1 ? 'bg-gray-50/50' : ''">
              ...
              <td class="px-5 py-3 text-center">
                <span v-if="row.videoTranscriber === true" class="text-success text-base">✓</span>
                <span v-else-if="row.videoTranscriber === false" class="text-text-muted">✗</span>
                <span v-else class="text-text-primary">{{ row.videoTranscriber }}</span>
              </td>
...
<script setup>
const rows = [
  { feature: '支持平台数量', videoTranscriber: '1800+', online: '10-50', desktop: '100-500' },
  { feature: 'AI 视频总结', videoTranscriber: true, online: false, desktop: false },
  { feature: '思维导图生成', videoTranscriber: true, online: false, desktop: false },
  { feature: '字幕下载', videoTranscriber: 'SRT/VTT/TXT', online: '部分支持', desktop: '部分支持' },
  { feature: '抖音无水印下载', videoTranscriber: true, online: '部分支持', desktop: '需登录' },
  { feature: '最高画质', videoTranscriber: '4K', online: '720p-1080p', desktop: '4K' },
  { feature: '无需安装', videoTranscriber: true, online: true, desktop: false },
  { feature: '手机浏览器可用', videoTranscriber: true, online: '部分支持', desktop: false },
  { feature: '费用', videoTranscriber: '免费', online: '免费/付费', desktop: '付费为主' },
]
</script>
