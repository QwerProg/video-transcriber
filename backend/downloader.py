...
class VideoDownloader:
    ...
    def extract_audio(self, url: str) -> tuple[str, float]:
        """
        下载视频的音频轨道，转为 Whisper 所需的格式。
        返回 (audio_path, duration_seconds)
        """
        import uuid
        audio_id = str(uuid.uuid4())[:8]
        output_template = os.path.join(self.DOWNLOAD_DIR, f"whisper_{audio_id}.%(ext)s")
        ...
        # 查找生成的音频文件
        audio_file = os.path.join(self.DOWNLOAD_DIR, f"whisper_{audio_id}.m4a")
        if not os.path.exists(audio_file):
            for ext in ["webm", "mp4", "mp3", "wav", "opus", "m4a"]:
                candidate = os.path.join(self.DOWNLOAD_DIR, f"whisper_{audio_id}.{ext}")
