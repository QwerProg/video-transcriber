...
def _get_transcriber():
    """延迟初始化 Transcriber（首次转录时加载 Whisper 模型）"""
    from transcriber import Transcriber
    if not hasattr(_get_transcriber, "_instance"):
        model_size = os.getenv("WHISPER_MODEL_SIZE", "base")
        _get_transcriber._instance = Transcriber(model_size=model_size)
    return _get_transcriber._instance


def _get_audio_downloader():
    """获取用于音频提取的 VideoDownloader 实例"""
    if not hasattr(_get_audio_downloader, "_instance"):
        _get_audio_downloader._instance = VideoDownloader()
    return _get_audio_downloader._instance
...
async def summarize_video(req: SummarizeRequest, user: dict | None = Depends(get_optional_user)) -> AsyncIterable[ServerSentEvent]:
    ...
    try:
        ...
        if not subtitle_data["has_subtitle"]:
            # 回退到 Whisper 语音转录
            try:
                audio_downloader = _get_audio_downloader()
                audio_path, duration = await loop.run_in_executor(
                    None, audio_downloader.extract_audio, req.url
                )
                transcriber = _get_transcriber()
                subtitle_data = await transcriber.transcribe(audio_path)
                # 清理临时音频文件
                try:
                ...
                yield ServerSentEvent(
                    raw_data=json.dumps(
                        {"message": f"该视频无可用字幕且语音转录失败: {str(e)}"},
                        ensure_ascii=False,
                    ),
                    event="error",
                )
