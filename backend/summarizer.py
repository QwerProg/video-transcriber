...
class VideoSummarizer:
    ...
    def summarize_stream(self, subtitle_text: str, language: str = "zh"):
        """流式生成视频总结，yield 每个 token。流式失败时回退到非流式。"""
        ...
        try:
            ...
            # 流式连接被 DeepSeek 服务端断开，回退到非流式请求
            print(f"[summarizer] 流式请求失败 ({e})，回退到非流式模式")
            response = self.client.chat.completions.create(
                model=self.model,
                messages=messages,
                stream=False,
    ...
    def chat_stream(self, subtitle_text: str, question: str):
        """基于视频内容的 AI 问答，流式返回。流式失败时回退到非流式。"""
        ...
        try:
            ...
            print(f"[summarizer] 流式问答失败 ({e})，回退到非流式模式")
            response = self.client.chat.completions.create(
                model=self.model,
                messages=messages,
                stream=False,
